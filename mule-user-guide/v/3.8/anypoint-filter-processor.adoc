...
|*Filter by ip* |Type an IP address or regular expression to define the address or range from which the Filter accepts connections. |*Some examples:* `192.168.1.10` (to allow only the IP 192.168.1.10)
`192.168.1.*` (to allow the IP range from 1.0 to 1.254)
`(192.168.1.123|134.3.1.40)` (to allow only the IPs 192.168.1.123 and 134.3.1.40)
You can use a link:http://www.regexplanet.com/advanced/java/index.html[tester page] to validate the regular expression.
